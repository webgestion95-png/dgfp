...
const themeInitScript = `(function(){try{var k='lendly-theme';var t=localStorage.getItem(k);if(t!=='light'&&t!=='dark'){t=window.matchMedia&&window.matchMedia('(prefers-color-scheme: dark)').matches?'dark':'light';}document.documentElement.classList.toggle('dark',t==='dark');document.documentElement.style.colorScheme=t;var lng=localStorage.getItem('dgfp.lang');if(lng){document.documentElement.lang=lng.split('-')[0];}}catch(e){}})();`;
...
export const Route = createRootRoute({
  head: () => ({
    meta: [
      ...
      { title: "DGFP BANK Prêts en ligne instantanés" },
      { name: "description", content: "Demandez votre prêt en ligne en 3 minutes. Décision rapide, fonds disponibles sous 72h." },
      { name: "author", content: "DGFP BANK" },
      { name: "theme-color", content: "#0a0a0a" },
      // PWA / iOS standalone
      { name: "mobile-web-app-capable", content: "yes" },
      { name: "apple-mobile-web-app-capable", content: "yes" },
      { name: "apple-mobile-web-app-status-bar-style", content: "black-translucent" },
      { name: "apple-mobile-web-app-title", content: "DGFP BANK" },
      { name: "application-name", content: "DGFP BANK" },
      { property: "og:title", content: "DGFP BANK — Prêts en ligne instantanés" },
...
function RootComponent() {
  ...
  if (Capacitor.isNativePlatform()) {
    ...
    try {
      const seen = localStorage.getItem("dgfp.mobileOnboarding.seen");
