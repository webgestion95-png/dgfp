...
function ReceiptPage() {
  ...
  async function downloadPdf() {
    ...
    try {
      ...
      a.href = url;
      a.download = `justificatif-virement-dgfp-${current.reference ?? current.id.slice(0, 8).toUpperCase()}.pdf`;
      a.click();
        ...
        <article className="receipt-paper rounded-2xl border border-border bg-white p-8 text-slate-900 shadow-elevated md:p-12">
          ...
          <header className="flex items-start justify-between border-b-2 border-[#db0011] pb-6">
            <div>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-semibold tracking-tight text-[#db0011]">DGFP BANK</p>
          ...
          <footer className="mt-12 border-t border-slate-200 pt-6 text-xs text-slate-500">
            ...
            <p className="mt-3 text-[10px] uppercase tracking-widest text-slate-400">
              DGFP BANK France · 38 av. Kléber, 75116 Paris · SIREN 775 670 284
            </p>
          </footer>
        </article>
      </div>
...
async function buildReceiptPdf(w: Withdrawal) {
  ...
  draw("DGFP BANK", margin + 46, height - 45, 18, bold, red);
  ...
  draw("Document généré par les systèmes sécurisés DGFP BANK.", margin + 16, 166, 9, bold, ink);
  ...
  draw("DGFP BANK France · 38 av. Kléber, 75116 Paris · SIREN 775 670 284", margin, 64, 8, regular, muted);
